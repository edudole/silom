...
async function load(){const status=document.getElementById('reportStatus');if(status){status.style.display='flex';status.innerHTML='<span class="report-spinner" aria-hidden="true"></span><span>กำลังรวบรวมรายการมาแสดง</span>';}try{const r=await fetch(`${API}?mode=reportitems&_t=${Date.now()}`,{cache:'no-store'}),j=await r.json();if(!r.ok||!j.success)throw new Error(j.message||'โหลดข้อมูลไม่สำเร็จ');state.items=Array.isArray(j.items)?j.items:[];render();}catch(e){if(status){status.style.display='block';status.textContent='โหลดรายงานไม่สำเร็จ: '+e.message;}const body=document.getElementById('reportTableBody');if(body)body.innerHTML='';}}
